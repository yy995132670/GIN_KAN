...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps,
        ...
        ### List of KANs (replacing MLPs)
        self.kans = torch.nn.ModuleList()

        ### List of batchnorms applied to the output of KAN (input of the final prediction linear layer)
        self.batch_norms = torch.nn.ModuleList()

        for layer in range(self.num_layers - 1):
            if layer == 0:
                self.kans.append(KAN([input_dim, hidden_dim, hidden_dim]))  # 替换为 KAN
            else:
                self.kans.append(KAN([hidden_dim, hidden_dim, hidden_dim]))  # 替换为 KAN
    ...
    def __preprocess_neighbors_maxpool(self, batch_graph):
        ### Create padded neighbor list in concatenated graph
    ...
    def next_layer_eps(self, h, layer, padded_neighbor_list=None, Adj_block=None):
        ...
        pooled_rep = self.kans[layer](pooled)  # 替换为 KAN
