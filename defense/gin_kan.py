...
class GIN_KAN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4, n_edge=1,with_relu=True, drop=False,
        ...
        num_features = nfeat
        dim = nhid
        nn1 = Sequential(KAN([num_features,dim, dim]), ReLU(),)
        self.gc1 = GINConv(nn1)
        # self.bn1 = torch.nn.BatchNorm1d(dim)
        nn2 = Sequential(KAN([dim,dim, dim]), ReLU(),)
        self.gc2 = GINConv(nn2)
        self.fc1 = KAN([dim, dim, dim])
        self.fc2 = KAN([dim,dim, int(nclass)])
    ...
    def forward(self, x, adj):
        ...
        x = self.gc2(x, edge_index=edge_index, edge_weight=adj_values)
